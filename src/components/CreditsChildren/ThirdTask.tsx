...
export default function ThirdTask() {
    ...
    let formula = Number(state.roubles) * (((Number(state.percent)/100)/Number(state.capitalization))/(1-(1+((Number(state.percent)/100)/Number(state.capitalization)))**(-(Number(state.years)*Number(state.capitalization)))))
  return (
    <div>
        ...
        <Input onChange={(e:ChangeEvent<HTMLInputElement>)=>{ChangeHandler(3, e.target.value)}} addonBefore="Размер ссуды - " addonAfter="руб." placeholder="" value={state.roubles ? state.roubles : ''} />
        ...
        >
            <span>Используя <Popover content={<MathComponent display = {false} tex={String.raw`A=R \frac{1-(1 + (\frac{i}{m}))^{-(n×m)}}{(\frac{i}{m})}`}/>} trigger="click">
        <span style={{color: "#5956ff", cursor: "pointer"}}>формулу </span>
        </Popover>
            :</span><br/>
        </ConfigProvider>
          <MathComponent display = {false} tex={String.raw`${state.roubles}=R \frac{1-(1 + (\frac{${Number(state.percent)/100}}{${state.capitalization}}))^{-(${state.years} × ${state.capitalization})}}{(\frac{${Number(state.percent)/100}}{${state.capitalization}})}`}/><br/>
