...
export default function FirstTask() {
    ...
    let formula = Number(state.roubles) * (((Number(state.percent)/100)/Number(state.capitalization))/((1+((Number(state.percent)/100)/Number(state.capitalization)))**(Number(state.years)*Number(state.capitalization))-1))
  return (
    <div>
        ...
        <Input onChange={(e:ChangeEvent<HTMLInputElement>)=>{ChangeHandler(3, e.target.value)}} addonBefore="Размер фонда - " addonAfter="руб." placeholder="" value={state.roubles ? state.roubles : ''} />
        ...
        <span style={{color: "#5956ff", cursor: "pointer"}}>формулу </span>
          ...
          <MathComponent display = {false} tex={String.raw`${state.roubles}=R \frac{(1 + (\frac{${Number(state.percent)/100}}{${state.capitalization}}))^{${state.years} × ${state.capitalization}}-1}{(\frac{${Number(state.percent)/100}}{${state.capitalization}})}`}/><br/>
